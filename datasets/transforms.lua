...
function M.CompressionNoise(low,high)
   return function(input)
      local low = low or 10
      local high = high or 90
      local comp_quality = (torch.rand(1)[1]*(high-low))+low -- 10 ~ 90
      return image.decompressJPG(image.compressJPG(input, comp_quality))
   end
end
...
function M.ScaleSquare(size, interpolation)
...
function M.DenseCrop(size)
   ...
   return function(input)
      ...
      for _, img in ipairs{input, image.hflip(input)} do
         --table.insert(output, centerCrop(img))
         table.insert(output, image.crop(img, 0, 0, size, size))
         table.insert(output, image.crop(img, (w-size)/2, 0, (w-size)/2+size, size))
         table.insert(output, image.crop(img, w-size, 0, w, size))
         table.insert(output, image.crop(img, 0, (h-size)/2, size, (h-size)/2+size))
         table.insert(output, image.crop(img, (w-size)/2, (h-size)/2, (w-size)/2+size, (h-size)/2+size))
         table.insert(output, image.crop(img, w-size, (h-size)/2, w, (h-size)/2+size))
         table.insert(output, image.crop(img, 0, h-size, size, h))
         table.insert(output, image.crop(img, (w-size)/2, h-size, (w-size)/2+size, h))
         table.insert(output, image.crop(img, w-size, h-size, w, h))
      end
...
-- center crop and horizontal flip
function M.CenterCrops(size)
   --local centerCrop = M.CenterCrop(size)
...
function M.RandomRotation(deg)
   return function(input)
      local random_deg = torch.uniform(-math.abs(deg),math.abs(deg))
      if random_deg ~= 0 then
         input = image.rotate(input, (torch.uniform() - 0.5) * random_deg * math.pi / 180, 'bilinear')
      end
      return input
   end
end
...
function M.rgb2gray()
   return function(input)
     local g = torch.zeros(1, input:size(2), input:size(3))
     g[1] = input[1] * 0.299 + input[2] * 0.587 + input[3]*0.114 
     input = g
     return input 
   end
end
