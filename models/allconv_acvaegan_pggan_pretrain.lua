...
local function createModel(opt)
   ...
   if opt.dataset == 'bird' or opt.dataset == 'celeba' then
      ...
      --encoder:add(DepthSeparable(baseChannels, baseChannels, 3, 8, 8, 8, eps, mom, opt))
      ...
      encoder:add(nn.SpatialBatchNormalization(baseChannels,eps,mom))
      encoder:add(nn.LeakyReLU(0.1))
      --encoder:add(DepthSeparable(baseChannels*2, baseChannels, 3, 8, 4, 4, eps, mom, opt))
      ...
      --encoder_attribute:add(nn.View(opt.batchSize*time_step, attribute_dim))
      ...
      --encoder_attribute:add(nn.View(opt.batchSize, time_step, latent_division, w, w))
      ...
      --decoder_attribute:add(nn.View(opt.batchSize*time_step, attribute_dim))
      ...
      --decoder_attribute:add(nn.View(opt.batchSize, time_step, w*w*latent_division))
      ...
      --prior:add(nn.View(opt.batchSize*time_step, w*w*latent_division))
   ...
   return {encoder_total, from_rgb_encoder, decoder, to_rgb, var_encoder, var_decoder, prior, attention, prior_attention_connection, attention_sum, from_rgb, gan_feature, recon, disc, sampling_z}, {KLD, ReconCriterion, StochasticCriterion, BCECriterion, ReconZCriterion}
end
