...
require 'utils.EntropyRegularization'
...
local vae_encoder, encoder_conv, from_rgb_encoder_layer, vae_decoder, decoder_conv, to_rgb_layer, var_encoder, var_decoder, prior, attention, attention_connection, attention_sum, from_rgb_layer, disc_conv, disc_feature, recon, disc, sampling_z = table.unpack(models)
local KLD, ReconCriterion, StochasticCriterion, BCECriterion, ReconZCriterion = table.unpack(criterions)
local sampling_z2 = sampling_z:clone()
local latent_division = opt.nf/opt.timeStep
local optimState_enc, optimState_from_rgb_enc, optimState_dec, optimState_to_rgb, optimState_var_enc, optimState_var_dec, optimState_prior, optimState_attn, optimState_from_rgb, optimState_disc, optimState_recon, optimState_feature

-- entropy regularization
local EntRegCriterion = nn.EntropyRegularization():cuda()
EntRegCriterion.sizeAverage = true
...
if opt.init_weight_from ~= 'none' then
  ...
  vae_encoder, from_rgb_encoder_layer, vae_decoder, to_rgb_layer, var_encoder, var_decoder, prior, attention, from_rgb_layer, disc_feature, recon, disc = table.unpack(models_resume)
  ...
  optimState_prior        = { learningRate = opt.LR_mult*opt.LR/opt.timeStep, optimize = true, numUpdates = 0}
  optimState_disc         = { learningRate = opt.LR_mult*opt.LR/2, beta1 = opt.beta1, optimize = true, numUpdates = 0, step = opt.discRatio}
  optimState_recon        = { learningRate = opt.LR_mult*opt.LR/2, beta1 = opt.beta1, optimize = true, numUpdates = 0}
  optimState_feature      = { learningRate = opt.LR_mult*opt.LR/2, beta1 = opt.beta1, optimize = true, numUpdates = 0}
  ...
  optimState_attn         = { learningRate = opt.LR_mult*opt.LR, optimize = true, numUpdates = 0}
elseif opt.retrain ~= 'none' then
  models_resume = torch.load(opt.retrain)
  if opt.optimState ~= 'none' then
    states_resume = torch.load(opt.optimState)
    optimState_enc, optimState_from_rgb_enc, optimState_dec, optimState_to_rgb, optimState_var_enc, optimState_var_dec, optimState_prior, optimState_attn, optimState_from_rgb, optimState_disc, optimState_recon, optimState_feature = table.unpack(states_resume)
  end
  vae_encoder, encoder_conv, from_rgb_encoder_layer, vae_decoder, decoder_conv, to_rgb_layer, var_encoder, var_decoder, prior, attention, from_rgb_layer, disc_conv, disc_feature, recon, disc = nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil
  vae_encoder, encoder_conv, from_rgb_encoder_layer, vae_decoder, decoder_conv, to_rgb_layer, var_encoder, var_decoder, prior, attention, from_rgb_layer, disc_conv, disc_feature, recon, disc = table.unpack(models_resume)
  collectgarbage()
else
  ...
  optimState_prior        = { learningRate = opt.LR/opt.timeStep, optimize = true, numUpdates = 0}
  optimState_disc         = { learningRate = opt.LR/2, beta1 = opt.beta1, optimize = true, numUpdates = 0, step = opt.discRatio}
  optimState_recon        = { learningRate = opt.LR/2, beta1 = opt.beta1, optimize = true, numUpdates = 0}
  optimState_feature      = { learningRate = opt.LR/2, beta1 = opt.beta1, optimize = true, numUpdates = 0}
  optimState_from_rgb     = { learningRate = opt.LR/2, beta1 = opt.beta1, optimize = true, numUpdates = 0}
  optimState_to_rgb       = { learningRate = opt.LR, optimize = true, numUpdates = 0}
  optimState_from_rgb_enc = { learningRate = opt.LR, optimize = true, numUpdates = 0}
  optimState_attn         = { learningRate = opt.LR, optimize = true, numUpdates = 0}
end
...
if opt.nGPU > 1 then
  ...
  attention = makeDataParallelTable(attention, opt.nGPU)
  attention_connection = makeDataParallelTable(attention_connection, opt.nGPU)
end
...
params_attn,          gradParams_attn         = attention:getParameters()
...
function train(opt)
  ...
  local function f_attention()            return 0.0, gradParams_attn; end
  local function f_attention_sample()     return 0.0, gradParams_attn; end
  ...
  attention:training(); attention_sum:training(); attention_connection:training()
  ...
  for t, sample in trainLoader:run() do
    ...
    local feature = disc_feature:forward({input_attr, merge_from_rgb.output})
    ...
    disc_feature:backward({input_attr, merge_from_rgb.output}, recon.gradInput)
    ...
    disc_feature:updateGradInput({input_attr, merge_from_rgb.output}, disc.gradInput)
    ...
    gradParams_prior = do_grad_clip(gradParams_prior, opt.grad_clip)
    ...
    local df_dvec = attention_connection:backward({input_attr, attention_vector}, prior.gradInput + var_decoder.gradInput[1] + vae_encoder.gradInput[1])
    --backprop doubly stochastic regularization
    local current_sum = attention_sum:forward(attention_vector)
    local loss_entreg = EntRegCriterion:forward(attention_vector)
    local df_entreg = EntRegCriterion:backward(attention_vector)
    local loss_stochastic = StochasticCriterion:forward(current_sum, torch.ones(current_sum:size(1), current_sum:size(2)):mul(opt.attrDim/opt.timeStep):cuda())
    local df_dstocastic   = StochasticCriterion:backward(current_sum, torch.ones(current_sum:size(1), current_sum:size(2)):mul(opt.attrDim/opt.timeStep):cuda())
    local df_dsum = attention_sum:backward(attention_vector, df_dstocastic)
    attention:backward(inputs_attention, opt.rho*df_dsum + opt.rho_entreg*df_entreg + df_dvec[2])

    optim[opt.optimization](f_attention, params_attn, optimState_attn)
    ...
    -- make another forward pass as attention network has been updated
    local attention_vector = attention:forward(inputs_attention)
    local attention_attr   = attention_connection:forward({input_attr, attention_vector})
    inputs[1] = attention_attr
    ...
    merge_from_rgb:forward(from_rgb.output)
    local feature_sample = disc_feature:forward({input_attr, merge_from_rgb.output})
    ...
    -- backward (attention)
    attention:zeroGradParameters();
    local df_dvec = attention_connection:backward({input_attr, attention_vector}, var_decoder.gradInput[1])
    attention:backward(inputs_attention, df_dvec[2])
    optim[opt.optimization](f_attention_sample, params_attn, optimState_attn)
    ...
    merge_from_rgb:forward(from_rgb.output)
    local outputs_feature = disc_feature:forward({input_attr, merge_from_rgb.output})
    ...
    disc_feature:backward({input_attr, merge_from_rgb.output}, disc.gradInput)
    merge_from_rgb:backward(from_rgb.output, disc_feature.gradInput[2])
...
function val(opt)
  ...
  vae_encoder:forward({val_attention_attr,from_rgb_encoder.output[2]})
  var_encoder:forward(vae_encoder.output)
  local val_latent_z = sampling_z:forward(var_encoder.output)
  
  var_decoder:forward({val_attention_attr,val_latent_z})
  ...
  --(2) test generation
  val_prior = prior:forward(val_attention_attr)
  ...
  params_attn,          gradParams_attn         = nil, nil
  if epoch % opt.epochStep == 0 then
    torch.save(opt.save .. 'models_' .. epoch .. '.t7', {vae_encoder:clearState(), encoder_conv:clearState(), from_rgb_encoder_layer:clearState(), vae_decoder:clearState(), decoder_conv:clearState(), to_rgb_layer:clearState(), var_encoder:clearState(), var_decoder:clearState(), prior:clearState(), attention:clearState(), from_rgb_layer:clearState(), disc_conv:clearState(), disc_feature:clearState(), recon:clearState(), disc:clearState()})
    torch.save(opt.save .. 'states_' .. epoch .. '.t7', {optimState_enc, optimState_from_rgb_enc, optimState_dec, optimState_to_rgb, optimState_var_enc, optimState_var_dec, optimState_prior, optimState_attn, optimState_from_rgb, optimState_disc, optimState_recon, optimState_feature})
  end
  if epoch > 80 and epoch % 5 == 0 then
    torch.save(opt.save .. 'models_' .. epoch .. '.t7', {vae_encoder:clearState(), encoder_conv:clearState(), from_rgb_encoder_layer:clearState(), vae_decoder:clearState(), decoder_conv:clearState(), to_rgb_layer:clearState(), var_encoder:clearState(), var_decoder:clearState(), prior:clearState(), attention:clearState(), from_rgb_layer:clearState(), disc_conv:clearState(), disc_feature:clearState(), recon:clearState(), disc:clearState()})
    torch.save(opt.save .. 'states_' .. epoch .. '.t7', {optimState_enc, optimState_from_rgb_enc, optimState_dec, optimState_to_rgb, optimState_var_enc, optimState_var_dec, optimState_prior, optimState_attn, optimState_from_rgb, optimState_disc, optimState_recon, optimState_feature})
  end 
  if epoch % opt.step == 0 then
    ...
    optimState_attn.learningRate          = optimState_attn.learningRate*opt.decayLR
  end
  params_enc,           gradParams_enc            = vae_encoder:getParameters()
  params_dec,           gradParams_dec            = vae_decoder:getParameters()
  params_var_enc,       gradParams_var_enc        = var_encoder:getParameters()
...
function generate(opt)
  ...
  for t = 1, data.test_im:size(1) do
    ...
    local val_inputs_attention = torch.ones(opt.batchSize, opt.attrDim, opt.timeStep):cuda()
    local val_attention_vector = attention:forward(val_inputs_attention)
    local val_attention_attr   = attention_connection:forward({inputs_attr, val_attention_vector})
    local val_prior = prior:forward(val_attention_attr)
    local val_latent_z = sampling_z:forward(val_prior)
    var_decoder:forward({val_attention_attr,val_latent_z})
    vae_decoder:forward(var_decoder.output)
