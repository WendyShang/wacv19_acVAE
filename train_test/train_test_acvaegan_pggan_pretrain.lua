...
local vae_encoder, from_rgb_encoder, vae_decoder, to_rgb, var_encoder, var_decoder, prior, attention, attention_connection, attention_sum, from_rgb, disc_feature, recon, disc, sampling_z = table.unpack(models)
...
if opt.retrain ~= 'none' then
  ...
  vae_encoder, from_rgb_encoder, vae_decoder, to_rgb, var_encoder, var_decoder, prior, attention, from_rgb, disc_feature, recon, disc = nil, nil, nil, nil, nil, nil, nil, nil
  vae_encoder, from_rgb_encoder, vae_decoder, to_rgb, var_encoder, var_decoder, prior, attention, from_rgb, disc_feature, recon, disc = table.unpack(models_resume)
  collectgarbage()
else
...
function train(opt)
  ...
  for t, sample in trainLoader:run() do
    ...
    local feature = disc_feature:forward({input_attr, from_rgb.output})
    ...
    disc_feature:backward({input_attr, from_rgb.output}, recon.gradInput)
    ...
    disc_feature:updateGradInput({input_attr, from_rgb.output}, disc.gradInput)
    ...
    local feature_sample = disc_feature:forward({input_attr, from_rgb.output})
    ...
    local outputs_feature = disc_feature:forward({input_attr, from_rgb.output})
    ...
    disc_feature:backward({input_attr, from_rgb.output}, disc.gradInput)
...
function val(opt)
  ...
  if epoch % opt.epochStep == 0 then
    torch.save(opt.save .. 'models_' .. epoch .. '.t7', {vae_encoder:clearState(), from_rgb_encoder:clearState(), vae_decoder:clearState(), to_rgb:clearState(), var_encoder:clearState(), var_decoder:clearState(), prior:clearState(), attention:clearState(), from_rgb:clearState(), disc_feature:clearState(), recon:clearState(), disc:clearState()})
